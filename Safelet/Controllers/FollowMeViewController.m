...
- (IBAction)didTapCheckMarkButton:(id)sender {
    ...
    [[User currentUser] startFollowMeToGuardians:point address:self.checkInLocationName selectedUsers:arrSelectedUsers completion:^(NSString * _Nullable objectId, NSError * _Nullable error) {
                                                 
                                                 [hud hideAnimated:YES];
                                                 if (error) {
                                                     [SLErrorHandlingController handleError:error];
                                                 } else {
                                                     [[NSUserDefaults standardUserDefaults] setObject:objectId forKey:kFollowMeObjectIdKeyPath];
                                                     [[NSUserDefaults standardUserDefaults] synchronize];
                                                     
                                                     [UIAlertController showSuccessAlertWithMessage:NSLocalizedString(@"Your guardians have been informed.",
                                                                                                                      @"Follow-me success")];
